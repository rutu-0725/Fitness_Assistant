...
def correct_feedback(model,video='0',input_csv="yoga_Dataset/angle.csv"):
    # Load video
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks is not None:
            ...
            with open(input_csv, 'r') as inputCSV:
                for row in csv.reader(inputCSV):
                    # print(len(row),row[12])
                    if row[12] == Name_Yoga_Classification: 
                        accurate_angle_lists = [float(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5]), float(row[6]), float(row[7]), float(row[8]), float(row[9]), float(row[10]), float(row[11])]
            # Tiền tố cần kiểm tra
            folder_path = 'yoga_Dataset/angle.csv'
            ...
            posture = "CORRECT" if correct_angle_count > 8 else "WRONG"
            ...
            fps_text = f"FPS: {1.0 / (time.time() - fps_time):.3f},{Name_Yoga_Classification}"  # Hiển thị FPS với 3 chữ số sau dấu thập phân
...
# def correct_feedback(model,  video='0', input_csv="teacher_yoga/angle_teacher_yoga.csv"):
#     # Load video
#     cap = cv2.VideoCapture(video)  # Replace with your video path
...
#             # Calculate angles for all relevant landmarks
#             left_wrist_angle = calculate_angle(landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value],
#                                             landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value],
#                                             landmarks[mp_pose.PoseLandmark.LEFT_INDEX.value],'1')
#             angles.append(left_wrist_angle)
...
#             probabilities = model.predict_proba([angles])
#             class_labels = model.classes_
#             check_accry_class = any(probabilities[0, i] > 0.5 for i in range(len(class_labels)))

#             # Read accurate angles for the selected pose
#             with open(input_csv, 'r') as inputCSV:
#                 for row in csv.reader(inputCSV):
#                     if row[12] == Name_Yoga_Classification : 
#                         accurate_angle_lists = [float(row[i]) for i in range(12)]
            
#             # # Display classification result
#             # display_classification(image, Name_Yoga_Classification, check_accry_class)
